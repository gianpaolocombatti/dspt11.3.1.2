...
def upsert_user(twitter_handle, nlp):
    try:
        ...
        user_tweets_func = partial(twitter_user.timeline, count=200, exclude_replies=True, include_rts=False, tweet_mode='extended' )
        if len(user_tweet_ids) > 0:
            last_tweet_stored = np.max()
            user_tweets = user_tweets_func(since_id=last_tweet_stored)
        else:
